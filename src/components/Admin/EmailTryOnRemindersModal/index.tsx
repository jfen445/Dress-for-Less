import React from "react";
import dayjs from "dayjs";
import Modal from "@/components/Modal";
import Button from "@/components/Button";
import { sendTryOnReminderEmails } from "@/api/admin";
import { formatTryOnTimeSlot } from "../../../../common/constants/tryOn";

type TryOnReminderRow = {
  _id: string;
  name: string;
  email: string;
  date: string;
  timeSlot: string;
};

interface EmailTryOnRemindersModalProps {
  isOpen: boolean;
  setOpen: React.Dispatch<React.SetStateAction<boolean>>;
  bookings: TryOnReminderRow[];
  onSent: (message: string) => void;
  onError: (message: string) => void;
}

const EmailTryOnRemindersModal = ({
  isOpen,
  setOpen,
  bookings,
  onSent,
  onError,
}: EmailTryOnRemindersModalProps) => {
  const [selectedIds, setSelectedIds] = React.useState<Set<string>>(new Set());
  const [isSending, setIsSending] = React.useState(false);

  React.useEffect(() => {
    if (isOpen) setSelectedIds(new Set());
  }, [isOpen]);

  const allSelected =
    bookings.length > 0 && selectedIds.size === bookings.length;

  const toggleAll = () => {
    if (allSelected) {
      setSelectedIds(new Set());
    } else {
      setSelectedIds(new Set(bookings.map((b) => b._id)));
    }
  };

  const toggleOne = (id: string) => {
    setSelectedIds((prev) => {
      const next = new Set(prev);
      next.has(id) ? next.delete(id) : next.add(id);
      return next;
    });
  };

  const handleSend = async () => {
    if (!selectedIds.size) return;
    setIsSending(true);
    try {
      const bookingIds = [...selectedIds];
      const res = await sendTryOnReminderEmails(bookingIds);
      onSent(res.data?.message ?? "Emails sent successfully");
      setOpen(false);
    } catch {
      onError("Failed to send emails. Please try again.");
    } finally {
      setIsSending(false);
    }
  };

  return (
    <Modal isOpen={isOpen} setOpen={setOpen}>
      <h2 className="text-lg font-semibold text-gray-900 mb-1">
        Send try-on reminders
      </h2>
      <p className="text-sm text-gray-500 mb-4">
        Select the try-on appointments to send a reminder email to.
      </p>

      <div className="overflow-y-auto max-h-[55vh] border border-gray-200 rounded-md">
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-50 sticky top-0">
            <tr>
              <th className="py-3 pl-4 pr-2 text-left">
                <input
                  type="checkbox"
                  checked={allSelected}
                  onChange={toggleAll}
                  className="rounded border-gray-300 text-pink-600"
                />
              </th>
              <th className="py-3 px-3 text-left font-semibold text-gray-700">
                Customer
              </th>
              <th className="py-3 px-3 text-left font-semibold text-gray-700">
                Date
              </th>
              <th className="py-3 px-3 text-left font-semibold text-gray-700">
                Time
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 bg-white">
            {bookings.length === 0 ? (
              <tr>
                <td colSpan={4} className="py-8 text-center text-gray-400">
                  No try-on appointments to display.
                </td>
              </tr>
            ) : (
              bookings.map((booking) => {
                const checked = selectedIds.has(booking._id);
                return (
                  <tr
                    key={booking._id}
                    className={`cursor-pointer hover:bg-gray-50 ${checked ? "bg-pink-50" : ""}`}
                    onClick={() => toggleOne(booking._id)}
                  >
                    <td className="py-3 pl-4 pr-2">
                      <input
                        type="checkbox"
                        checked={checked}
                        onChange={() => toggleOne(booking._id)}
                        onClick={(e) => e.stopPropagation()}
                        className="rounded border-gray-300 text-pink-600"
                      />
                    </td>
                    <td className="py-3 px-3">
                      <div className="font-medium text-gray-900">
                        {booking.name}
                      </div>
                      <div className="text-gray-500 text-xs">
                        {booking.email}
                      </div>
                    </td>
                    <td className="py-3 px-3 text-gray-600 whitespace-nowrap">
                      {dayjs(booking.date).format("MMM D, YYYY")}
                    </td>
                    <td className="py-3 px-3 text-gray-600">
                      {formatTryOnTimeSlot(booking.timeSlot)}
                    </td>
                  </tr>
                );
              })
            )}
          </tbody>
        </table>
      </div>

      <div className="mt-4 flex items-center justify-between">
        <span className="text-sm text-gray-500">
          {selectedIds.size} of {bookings.length} selected
        </span>
        <div className="flex gap-3">
          <Button
            type="button"
            variant="ghost"
            onClick={() => setOpen(false)}
            className="rounded-md px-4 py-2 text-sm text-gray-700 ring-1 ring-inset ring-gray-300 hover:bg-gray-50"
          >
            Cancel
          </Button>
          <Button
            onClick={handleSend}
            disabled={selectedIds.size === 0 || isSending}
          >
            {isSending
              ? "Sending…"
              : `Send${selectedIds.size > 0 ? ` (${selectedIds.size})` : ""}`}
          </Button>
        </div>
      </div>
    </Modal>
  );
};

export default EmailTryOnRemindersModal;
